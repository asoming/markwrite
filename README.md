...
1. **打开或新建。** 在「文件」选择「打开文档…」「打开文件夹…」或「新建文档」。打开文档默认进入阅读模式；要修改内容，选择「视图 → 即时渲染编辑」。
...
| 分类   | 可设置内容                                                                          |
| ------ | ----------------------------------------------------------------------------------- |
| 文件   | 自动保存、Markdown 默认打开方式；查看搜索排除规则                                   |
...
默认打开模式为**阅读**，也可改为即时渲染编辑或 Markdown 源码。当前文档仍可通过「视图」随时切换。切换界面语言不翻译或修改文档内容。旧设置会补齐新增字段，保留已有排版和保存偏好。

### 主题

保留浅色、深色、跟随系统，并增加以下五种主题。菜单「主题」和设置「外观」均可选择。

| 主题      | 外观                                 |
| --------- | ------------------------------------ |
| Github    | 白底、清晰标题分隔和蓝色链接         |
| Newsprint | 纸张底色、衬线正文和较明显的标题横线 |
| Night     | 深蓝底色、柔和文字与浅青色链接       |
| Pixyll    | 衬线正文与醒目的无衬线标题           |
| Whitey    | 中性白底、细线分隔与绿色强调色       |

这些名称用于区分本应用的原创视觉预设，没有复制 Typora 的主题资产。主题同时作用于阅读和编辑正文；切换主题会恢复该主题的颜色和字体风格，保留自定义字体名称、字号、行高和宽度。已指定的自定义字体需要在本机安装，否则使用备用字体。
...
在「设置 → 文件」点击「将 Markdown 默认设为墨页打开」。

- **Linux：**创建或更新当前用户的 Markwrite 入口，通过 `xdg-mime` 设置 `text/markdown` 与 `text/x-markdown`，并查询结果。需要桌面会话和 `xdg-utils`。此操作不改变 TXT、HTML、DOCX 的默认应用。
- **Windows：**注册 Markwrite 为可选应用，再打开系统「默认应用」设置。需要在那里为 `.md`、`.markdown` 选择 Markwrite；打开系统设置本身不等于关联已经完成。

便携目录移动后请重新生成入口或重新设置关联，避免旧入口继续指向原位置。

## 编辑与阅读

顶部提供「文件、编辑、段落、格式、插入、视图、主题、工具、帮助」菜单，常用操作不依赖记忆语法或快捷键。
...
- 打开单篇文件或工作文件夹，使用文件树、多标签、最近文件；支持中文与空格路径、原生拖入、第二次启动时向已有窗口转交文件。
- 创建文档/文件夹、重命名、批量移到系统回收站。未保存修改会阻止相关删除；重命名不会自动重写其他文档的引用。
- 记住最近 10 个工作目录，可在侧栏切换；这是目录切换，不是跨工作区同步或聚合搜索。
...
扫描仅针对当前授权工作目录，排除隐藏目录、`.git`、`node_modules`、`target` 和符号链接；全文搜索最多显示 500 条匹配。
...
设置中的默认应用按钮会打开 Windows 系统设置，请在那里完成 `.md` 和 `.markdown` 关联。当前 0.3 代码的 Windows 构建、安装、启动关闭及实际桌面行为仍需以对应版本的验证结果为准。
...
```bash
sudo apt install build-essential pkg-config libwebkit2gtk-4.1-dev libgtk-3-dev librsvg2-dev patchelf xdg-utils desktop-file-utils
npm ci
npm run tauri dev
```
