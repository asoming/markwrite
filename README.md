...
**1 秒首屏仍是优化目标，尚未作为达标承诺。** 1 MiB 文档在私有 Xephyr 软件渲染环境中，从启动进程到辅助功能接口读到正文，单次样本为 **1.339 秒**（250ms 轮询）；小文档源码编辑样本为 1.489 / 1.818 秒。该测量不是普通桌面硬件上的重复冷启动基准；单文件上限与巨型单块、复杂图表的成本仍然存在。
...
**The one-second target remains unproven.** One 1MiB sample took **1.339 seconds** from process launch to accessible reader content under private Xephyr software rendering with 250ms polling; small source-editor samples took 1.489 / 1.818 seconds. These are individual instrumented samples, not repeated cold-start benchmarks on normal desktop hardware. Single-file limits and unusually large individual blocks or diagrams still apply.
