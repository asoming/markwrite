...
- `Ctrl+1` 编辑、`Ctrl+2` 源码、`Ctrl+3` 阅读；`Ctrl+Shift+M` 循环切换。Mac 使用 `⌘`。并排时作用于当前聚焦的一侧。
- **设置 → 快捷键**：点击操作右侧按钮，按下组合键，点击「应用快捷键」。支持清除、恢复默认与冲突检查，重启后仍保留。系统保留的按键可能由操作系统优先处理。
- 普通启动不重开历史文档或文件夹。冷启动双击文件时只打开指定文件；应用运行期间继续打开文件才增加标签。未保存草稿独立保留，通过 **文件 → 恢复未保存草稿** 手动取回。显式「恢复独立窗口」仍恢复所选窗口。
- 更新检查默认无需令牌；GitHub API 无法使用时，稳定版自动尝试公开发布的 `update.json`。下载仍检查文件大小和 SHA-256。限流、无效令牌、资源不存在分别提示；预览版需要 API 正常可用。

![快捷键录制、修改与保存反馈](screenshots/custom-shortcuts.png)
